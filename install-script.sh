...
mkdir -p ~/.local/bin
mkdir -p ~/.config/recordatori-descans
mkdir -p ~/.config/autostart

# Copiar l'script
echo "Instal·lant l'script principal..."
cat > ~/.local/bin/recordatori-descans.py << 'EOL'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
chmod +x ~/.local/bin/recordatori-descans.py

# Crear l'entrada d'autoarranc
echo "Configurant autoarranc..."
cat > ~/.config/autostart/recordatori-descans.desktop << EOL
[Desktop Entry]
Type=Application
Name=Recordatori de Descans
Comment=Aplicació de recordatori de descansos saludables
Exec=python3 $HOME/.local/bin/recordatori-descans.py --autostart
Terminal=false
Categories=Utility;
StartupNotify=false
Hidden=false
...
cat > ~/.local/share/applications/recordatori-descans.desktop << EOL
[Desktop Entry]
Type=Application
Name=Recordatori de Descans
Comment=Aplicació de recordatori de descansos saludables
Exec=python3 $HOME/.local/bin/recordatori-descans.py
...
echo "python3 ~/.local/bin/recordatori-descans.py"
echo ""
echo "Vols iniciar l'aplicació ara mateix? (s/n)"
read -r resposta
if [[ $resposta =~ ^[Ss]$ ]]; then
    echo "Iniciant l'aplicació..."
    python3 ~/.local/bin/recordatori-descans.py &
